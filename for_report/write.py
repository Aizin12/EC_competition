...
def make_scatterplot(result):
    ...
    for i in range(size):
        ...
        x = np.arange(G+1)
        ax.plot(x,logscale(result[i,:int(G+1)]),label=make_name(i),marker="o",markersize=1)
...
cnt = 0
trails = 33
data = np.zeros((size,10001,trails+1))
for subject in args.subject:
    G = 10**5/int(parameter[cnt][5])
    data[cnt,:int(G)+1,:] = read_file(subject,int(G),trails)
    cnt += 1
